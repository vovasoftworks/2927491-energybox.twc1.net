	...
	public function execute(Schedule_Model $oSchedule)
	{
		...
		if ($oModule->active)
		{
			...
			Core_Log::instance()
				...
				->write(Core::_('Schedule.log_message_begin', $oModule->name, $oSchedule->entity_id, $oSchedule->action));
			...
			Core_Log::instance()
				->status(Core_Log::$MESSAGE)
				->write(Core::_('Schedule.log_message_end', $oModule->name, $oSchedule->entity_id, $oSchedule->action));
		}
		else
		{
			...
			Core_Log::instance()
				->status(Core_Log::$MESSAGE)
				->write(Core::_('Schedule.log_message_disabled', $oModule->name, $oSchedule->entity_id, $oSchedule->action));
		}
	}
